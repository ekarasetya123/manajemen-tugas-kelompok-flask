    ...
    with app.app_context():
        ...
        if Tugas.query.count() == 0:
            ...
            if mk_ids:
                ...
                tugas_data = [
                    ('Tugas 1 Kewarganegaraan', 'Buat esai tentang hak dan kewajiban warga negara',
                     mk_ids[0] if len(mk_ids) > 0 else None, today + timedelta(days=2), 'belum'),
                    ('Latihan Logika', 'Soal‑soal tentang proposisi dan bukti indirekt',
                     mk_ids[1] if len(mk_ids) > 1 else None, today + timedelta(days=5), 'proses'),
                    ('Algoritma Sorting', 'Implementasi quicksort dan mergesort dalam Python',
                     mk_ids[2] if len(mk_ids) > 2 else None, today + timedelta(days=3), 'selesai'),
                    ('Integral Lipat Ganda', 'Hitung luas daerah terkena fungsi f(x,y)',
                     mk_ids[3] if len(mk_ids) > 3 else None, today + timedelta(days=1), 'belum'),
                    ('Presentasi Transformasi Digital', 'Buat slide 10 halaman tentang dampak AI',
                     mk_ids[4] if len(mk_ids) > 4 else None, today + timedelta(days=7), 'proses'),
                    ('Hello World', 'Buat program saperta console output',
                     mk_ids[5] if len(mk_ids) > 5 else None, today + timedelta(days=4), 'belum'),
                    ('Inheritance dan Polymorphism', 'Buat kelas Hewan dan turunannya',
                     mk_ids[6] if len(mk_ids) > 6 else None, today + timedelta(days=6), 'proses'),
                    ('Ejaan dan Struktur Paragraf', 'Latihan membuat paragraph efektif',
                     mk_ids[7] if len(mk_ids) > 7 else None, today + timedelta(days=0), 'selesai')
                ]
